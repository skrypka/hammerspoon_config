   ...
   if (status < 100) or (status >= 400) then
      ...
      if json then
         ...
         for i,v in ipairs(json) do
            ...
            v.svn_url = self.repos[repo].svn_base_url .. v.name .. ".spoon"
...
function obj:_build_repo_json_url(repo)
   if self.repos[repo] and self.repos[repo].url then
      ...
      self.repos[repo].svn_base_url = self.repos[repo].url .. "/trunk/Source/"
      return true
   else
...
--- SpoonInstall:installSpoonFromSVN(name, repo)
--- Method
--- Synchronously install a Spoon from a registered repository
---
--- Parameters:
...
--- Returns:
---  * `true` if the installation was successful, `nil` otherwise.
function obj:installSpoonFromSVN(name, repo)
   if not repo then repo = 'default' end
   if self:_is_valid_spoon(name, repo) then
      local command = string.format("svn checkout %s %s/Spoons/%s.spoon", self.repos[repo].data[name].svn_url, hs.configdir, name)
      output, status = hs.execute(command)
      print("SVN checkout", command)
      print("SVN result", output, status)
      return true
   end
   return nil
end
...
---    * svn - if `true`, the Spoon will be installed from Source folder without zip. Good to use for development
---
--- Returns:
---  * None
   ...
   if hs.spoons.use(name, arg, true) then
      ...
      if self.repos[repo] then
         if self.repos[repo].data then
            ...
            if arg.svn then
               return load_and_config(nil, self:installSpoonFromSVN(name, repo))
            elseif self.use_syncinstall then
